-- migrate:up
-- Baseline: pg_dump --schema-only of AWS India production (Aurora PG 16.6,
-- prod_flexprice) taken 2026-08-19. See migrations/baseline/README.md for the
-- deliberate deviations (search DEVIATION FROM PROD in that file).
--
-- Existing deployments must NOT run this. Adopt them with:
--   ./scripts/migrations/adopt.sh <url> migrations/versioned/postgres 20260819000000
-- which records the version and executes nothing.
...
-- The pg_dump prologue runs set_config('search_path','',false) — the false makes it
-- session-wide, not transaction-local. dbmate reuses one connection for the whole
-- run, so without this every later migration referencing an unqualified table fails
-- with 'relation does not exist'.
SELECT pg_catalog.set_config('search_path', 'public', false);

-- migrate:down
-- Baseline is not reversible. Restore from a snapshot.
