...
### 6.1 Enhancements needed

- **Make a finalized `tax_applied` record immutable in the schema, not by convention.**  
  4.12 freezes a finalized row through a status check in application code, which holds until a caller is added that does not know about it. An external row is not only our record: it corresponds to a transaction in the provider's books that the tenant has filed on, so drift between the two reconciles against nothing.

  The end state is that a row is immutable apart from `tax_transaction_id`. That column alone is updatable after the row is written, because commit changes nothing about the tax: it records that the amounts already on the row were filed.

  Everything else, `tax_calculation_id` included, is fixed at write time. A calculation ID identifies the calculation those amounts came from, so a new one means a new calculation and potentially different amounts. The two cannot be separated. Any recalculation therefore archives the existing rows and writes a new set carrying the new calculation ID and its amounts together, rather than updating a row in place. A row's amounts are then true for the life of that row instead of being the latest of several values it has held.

  This is deferred rather than shipped because it changes the write path for native rows as well, which today update in place, and because archive-and-rewrite grows the table on every draft recompute and needs a retention answer of its own.
