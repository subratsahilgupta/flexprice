...
const (
	...
	WorkflowMoyasarInvoiceSync = "MoyasarInvoiceSyncWorkflow"
	// Activity names - must match the registered method names
	ActivitySyncInvoiceToMoyasar = "SyncInvoiceToMoyasar"
)

// MoyasarInvoiceSyncWorkflow orchestrates the Moyasar invoice synchronization process
// Steps:
...
func MoyasarInvoiceSyncWorkflow(ctx workflow.Context, input models.MoyasarInvoiceSyncWorkflowInput) error {
	...
	// Step 2: Sync invoice to Moyasar
	logger.Info("Step 2: Syncing invoice to Moyasar", "invoice_id", input.InvoiceID)

	err = workflow.ExecuteActivity(ctx, ActivitySyncInvoiceToMoyasar, input).Get(ctx, nil)
	if err != nil {
		logger.Error("Failed to sync invoice to Moyasar",
			"error", err,
			"invoice_id", input.InvoiceID,
			"customer_id", input.CustomerID)
		return err
	...
	logger.Info("Successfully completed Moyasar invoice sync workflow",
		"invoice_id", input.InvoiceID,
		"customer_id", input.CustomerID)
