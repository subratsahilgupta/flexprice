...
func extractWorkflowContext(args []interface{}) (tenantID, environmentID, userID, entity, entityID string, metadata map[string]interface{}) {
	defer func() {
		if r := recover(); r != nil {
			tenantID = ""
			environmentID = ""
	...
	for i := 0; i < val.NumField(); i++ {
		...
		// Prefer explicit user id field if present on workflow input.
		// Many workflow inputs include UserID for audit, or DefaultUserID as *string.
		if field.Name == "UserID" && fieldVal.Kind() == reflect.String {
			userID = fieldVal.String()
			continue
		}
		if field.Name == "DefaultUserID" && fieldVal.Kind() == reflect.Ptr && !fieldVal.IsNil() && fieldVal.Elem().Kind() == reflect.String {
			userID = fieldVal.Elem().String()
			continue
		}
	...
	return tenantID, environmentID, userID, entity, entityID, metadata
}
