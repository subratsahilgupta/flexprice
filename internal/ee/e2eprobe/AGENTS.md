...
- **Commitments / coupon on cust #1**: attached at sub-create only. Existing tenants whose sub already exists WON'T get these retro-fitted — the seed doesn't mutate existing subs (that would break `cycle-invoice-probe`'s baseline). Fresh tenants get full coverage from day one; older tenants continue running until their sub is naturally recreated. The shared coupon is ONCE cadence, so `persistent-billing-invariants-probe` asserts the oldest subscription invoice (`order=asc`, `limit=1`) and soft-skips when the sub has no coupon association.
...
**File layout**
- New probe → `internal/ee/e2eprobe/checks/<snake_name>.go` + `_test.go`
- New sub-interface method on `Client` → `internal/ee/e2eprobe/client.go`, `client_dryrun.go`, `client_dryrun_test.go`, `checks/fakeclient_test.go`
- Register the check name → `internal/ee/e2eprobe/config.go` (`CheckNames` + `checkDefaultIntervals`)
- Wire it up → `cmd/e2eprobe/main.go`
- Update ops docs → `internal/ee/e2eprobe/README.md` (checks table + self-provisioning list) + `cmd/e2eprobe/.env.example` (new env vars for enabled/interval)
...
`internal/ee/e2eprobe/client.go` wraps the SDK behind an in-repo `Client`
