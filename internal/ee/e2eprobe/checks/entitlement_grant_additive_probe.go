...
func (p *EntitlementGrantAdditiveProbe) Run(ctx context.Context) error {
	...
	if _, err := p.client.Customers().Create(ctx, types.CreateCustomerRequest{
		ExternalID: ext,
		Name:       "E2EProbe Ephemeral Grant Additive",
...
func (p *EntitlementGrantAdditiveProbe) pollRawEvents(ctx context.Context, ext, subID, featID string) error {
	...
	pageSize := int64(200)
	// Staging's ingest consumer drains ~1.5 events/s. This probe's 200-event
	// burst plus concurrent probes (550 for commitment, 100 for tax) share
	// that queue. 30s and 90s both flaked; 180s covers a few hundred queued.
