...
// TestCalculateFixedCharges_MixedCadence_ZeroDurationLineItemNotCharged verifies that a
// longer-cadence (quarterly) line item whose StartDate == EndDate (zero duration — reachable via
// backdating a line item's end date to itself) is never charged the full cadence price, even when
// its matched period falls inside the invoice window.
func (s *BillingServiceSuite) TestCalculateFixedCharges_MixedCadence_ZeroDurationLineItemNotCharged() {
	ctx := s.GetContext()
	jan1 := time.Date(2024, time.January, 1, 0, 0, 0, 0, time.UTC)
	apr1 := time.Date(2024, time.April, 1, 0, 0, 0, 0, time.UTC)
	may1 := time.Date(2024, time.May, 1, 0, 0, 0, 0, time.UTC)
	...
	cust := &customer.Customer{
		ID:         "cust_zero_dur",
		ExternalID: "ext_zero_dur",
		Name:       "Zero Duration Customer",
		Email:      "zerodur@example.com",
		BaseModel:  types.GetDefaultBaseModel(ctx),
	}
	s.NoError(s.GetStores().CustomerRepo.Create(ctx, cust))
	pl := &plan.Plan{
		ID:          "plan_zero_dur",
		Name:        "Zero Duration Plan",
		Description: "Zero duration line item test",
		BaseModel:   types.GetDefaultBaseModel(ctx),
	}
	s.NoError(s.GetStores().PlanRepo.Create(ctx, pl))

	priceQuarterly := &price.Price{
		ID:                 "price_quarterly_zero_dur",
		Amount:             decimal.NewFromInt(300),
		Currency:           "usd",
		EntityType:         types.PRICE_ENTITY_TYPE_PLAN,
		EntityID:           pl.ID,
	...
	// Quarterly line item backdated so its EndDate equals its StartDate (zero duration).
	// Advance rule: matched period start (Apr 1) falls in the invoice window [Apr 1, May 1).
	liQuarterlyZeroDuration := &subscription.SubscriptionLineItem{
		ID:                 types.GenerateUUIDWithPrefix(types.UUID_PREFIX_SUBSCRIPTION_LINE_ITEM),
		SubscriptionID:     sub.ID,
		CustomerID:         sub.CustomerID,
		EntityID:           pl.ID,
	...
	s.NoError(s.GetStores().SubscriptionRepo.CreateWithLineItems(ctx, sub, []*subscription.SubscriptionLineItem{liQuarterlyZeroDuration}))
	sub.LineItems = []*subscription.SubscriptionLineItem{liQuarterlyZeroDuration}

	result, err := s.service.CalculateFixedCharges(ctx, &dto.CalculateFixedChargesParams{Subscription: sub, PeriodStart: apr1, PeriodEnd: may1})
	s.NoError(err)
	s.Empty(result.LineItems, "a zero-duration line item must never be charged the full cadence price")
	s.True(result.TotalAmount.IsZero(), "total must be zero when the only line item has zero duration")
}
