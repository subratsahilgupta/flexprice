...
// A calendar subscription that starts mid-period prorates addon credit grants with the
// opening invoice. Line-item proration stays off on this path so the addon is not billed twice.
func (s *SubscriptionServiceSuite) TestCreateSubscription_CalendarProratesAddonCreditGrant() {
	ctx := s.GetContext()
	addonID := "addon_cg_create_calendar"
	s.setupCreditGrantAddon(addonID, 100, types.CreditGrantCadenceRecurring)
	...
	// The cycle this start lands in runs 10-01 → 11-01; the subscription only covers its tail.
	// Spelled out rather than recomputed with the helper under test, or the assertion would
	// agree with any window the implementation picked.
	fullStart := time.Date(2026, 10, 1, 0, 0, 0, 0, time.UTC)
	s.WithinDuration(time.Date(2026, 11, 1, 0, 0, 0, 0, time.UTC), sub.CurrentPeriodEnd, time.Second)

	coefficient, err := proration.Coefficient(fullStart, sub.CurrentPeriodEnd, start, types.StrategySecondBased)
	s.Require().NoError(err)

	grant := s.addonGrantOn(sub.ID, addonID)
	s.Require().NotNil(grant)
	s.WithinDuration(sub.CurrentPeriodEnd, lo.FromPtr(grant.CreditGrantAnchor), time.Second)
	s.True(grant.Credits.Equal(decimal.NewFromInt(100)), "the grant keeps its full amount; only the first application is scaled")

	app := s.firstApplicationFor(grant.ID)
	s.Require().NotNil(app)
	s.Require().NotNil(app.PeriodEnd)
	s.WithinDuration(sub.CurrentPeriodEnd, lo.FromPtr(app.PeriodEnd), time.Second)
	expected := decimal.NewFromInt(100).Mul(coefficient).Round(creditGrantCreditsScale)
	s.True(expected.LessThan(decimal.NewFromInt(100)))
	s.Equal(expected.String(), app.Credits.String())
	s.Equal(coefficient.String(), app.Metadata["proration_coefficient"])
	s.Equal("100", app.Metadata["proration_original_credits"])
	s.Equal("addon_attach", app.Metadata["proration_source"])
}

func (s *SubscriptionServiceSuite) TestCreateSubscription_AddonCreditGrantStaysFullWithoutProration() {
	ctx := s.GetContext()
	addonID := "addon_cg_create_full"
	s.setupCreditGrantAddon(addonID, 100, types.CreditGrantCadenceRecurring)
	...
	grant := s.addonGrantOn(resp.ID, addonID)
	s.Require().NotNil(grant)
	app := s.firstApplicationFor(grant.ID)
	s.Require().NotNil(app)
	s.True(app.Credits.Equal(decimal.NewFromInt(100)))
	s.Empty(app.Metadata["proration_coefficient"])
}

// materializedAddonGrant looks up the fixture subscription. addonGrantOn looks up any subscription.
func (s *SubscriptionServiceSuite) addonGrantOn(subID, addonID string) *creditgrant.CreditGrant {
	subService := s.service.(*subscriptionService)
	filter := types.NewNoLimitCreditGrantFilter()
