...
type Repository interface {
	...
	// SubscriptionsWithProvisionalFacts returns the subscriptions that hold any
	// provisional facts. The rollup scan uses it to find subscriptions it has
	// never rolled: on every other signal those are indistinguishable from a
	// subscription that is simply up to date.
	SubscriptionsWithProvisionalFacts(ctx context.Context) ([]string, error)
}
