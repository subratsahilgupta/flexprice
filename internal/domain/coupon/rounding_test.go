...
// TestDiscountRounding_StackingDiscounts tests multiple discounts applied
// sequentially (as would happen with line-item + invoice-level discounts)
func TestDiscountRounding_StackingDiscounts(t *testing.T) {
	t.Run("LineItem_Then_Invoice_Discount", func(t *testing.T) {
		// Scenario: $100 item with 10% line-item discount, then 15% invoice discount
		// Step 1: $100 - 10% = $100 - $10.00 = $90.00
		// Step 2: $90 - 15% = $90 - $13.50 = $76.50
		// Total discount: $10.00 + $13.50 = $23.50

		originalPrice := decimal.NewFromFloat(100.00)

		// First discount: 10% line-item
		lineItemPercentage := decimal.NewFromFloat(10)
		lineItemCoupon := &Coupon{
			Type:          types.CouponTypePercentage,
			PercentageOff: &lineItemPercentage,
		}

		result1 := lineItemCoupon.ApplyDiscount(originalPrice, "usd")

		expectedDiscount1 := decimal.NewFromFloat(10.00)
		expectedPrice1 := decimal.NewFromFloat(90.00)
		assert.True(t, result1.Discount.Equal(expectedDiscount1))
		assert.True(t, result1.FinalPrice.Equal(expectedPrice1))

		// Second discount: 15% invoice-level on remaining amount
		invoicePercentage := decimal.NewFromFloat(15)
		invoiceCoupon := &Coupon{
			Type:          types.CouponTypePercentage,
			PercentageOff: &invoicePercentage,
		}

		result2 := invoiceCoupon.ApplyDiscount(result1.FinalPrice, "usd")

		expectedDiscount2 := decimal.NewFromFloat(13.50)
		expectedFinal := decimal.NewFromFloat(76.50)
		assert.True(t, result2.Discount.Equal(expectedDiscount2))
		assert.True(t, result2.FinalPrice.Equal(expectedFinal))

		// Total discount
		totalDiscount := result1.Discount.Add(result2.Discount)
		expectedTotal := decimal.NewFromFloat(23.50)
		assert.True(t, totalDiscount.Equal(expectedTotal))
	})

	t.Run("Two_Fixed_Discounts", func(t *testing.T) {
		// $100 item with $10 discount, then $5 discount
		originalPrice := decimal.NewFromFloat(100.00)

		firstDiscount := decimal.NewFromFloat(10.00)
		firstCoupon := &Coupon{
			Type:      types.CouponTypeFixed,
			AmountOff: &firstDiscount,
		}

		result1 := firstCoupon.ApplyDiscount(originalPrice, "usd")
		assert.True(t, result1.FinalPrice.Equal(decimal.NewFromFloat(90.00)))

		secondDiscount := decimal.NewFromFloat(5.00)
		secondCoupon := &Coupon{
			Type:      types.CouponTypeFixed,
			AmountOff: &secondDiscount,
		}

		result2 := secondCoupon.ApplyDiscount(result1.FinalPrice, "usd")
		assert.True(t, result2.FinalPrice.Equal(decimal.NewFromFloat(85.00)))

		totalDiscount := result1.Discount.Add(result2.Discount)
		assert.True(t, totalDiscount.Equal(decimal.NewFromFloat(15.00)))
	})
}
