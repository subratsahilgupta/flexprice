...
func NewRouter(
	...
	admin := permissionMW.RequireSuperAdmin // shorthand used on routes accessible only to super_admin users
	...
	v1Private.Use(middleware.ErrorHandler())
	{
		user := v1Private.Group("/users")
		{
			...
			user.PUT("/:id/roles", admin(types.EntityUser, types.ActionWrite), handlers.User.UpdateUserRoles)
