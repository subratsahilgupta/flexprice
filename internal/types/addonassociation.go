...
type AddonAssociationFilter struct {
	...
	*TimeRangeFilter
	...
	AssociationIDs []string                    `json:"association_ids,omitempty" form:"association_ids" validate:"omitempty"`
...
func (f *AddonAssociationFilter) Validate() error {
	...
	for _, associationID := range f.AssociationIDs {
		if associationID == "" {
			return ierr.NewError("association id can not be empty").
				WithHint("Association info can not be empty").
				Mark(ierr.ErrValidation)
		}
	}
