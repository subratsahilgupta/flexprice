...
var googleEndpointSuffixes = []string{
	".googleapis.com",
	".google.com",
}

// validateGoogleEndpoint checks a URL from a workload identity federation
// config is https and served from a Google domain.
func validateGoogleEndpoint(raw, field string) error {
	...
	host := strings.ToLower(u.Hostname())
	for _, suffix := range googleEndpointSuffixes {
		if strings.HasSuffix(host, suffix) {
			return nil
		}
	}

	return ierr.NewError(field + " must be a Google endpoint").
		WithHintf("GCP Marketplace credentials_json %s must be a Google endpoint — paste the file generated by `gcloud iam workload-identity-pools create-cred-config` unmodified", field).
		WithReportableDetails(map[string]any{
			"allowed_domains": googleEndpointSuffixes,
		}).
		Mark(ierr.ErrValidation)
}
