...
// The CustomPricingUnitFunc type is an adapter to allow the use of ordinary
// function as CustomPricingUnit mutator.
type CustomPricingUnitFunc func(context.Context, *ent.CustomPricingUnitMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CustomPricingUnitFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CustomPricingUnitMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CustomPricingUnitMutation", m)
}
